//
//  MortScene1ViewController.swift
//  Oasis
//
...
class MortScene1ViewController: UIViewController, UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout, CheckboxDelegate, MortViewControllerDelegate {
    @IBOutlet weak var btnBottom: ButtonNoIcon!
    ...
    func makeData(){
        mvc.popMorts(result: 2)
    }
    func mortsResponse() {
        
        morts = realm.objects(Mort.self).filter("flag_result = \"2\"")
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if let obj: Mort = morts[ indexPath.row ]{
            ...
            if obj.history != "" { cell.history.text = "사유: \(obj.history)" }
            else { cell.history.isHidden = true }
